...
nested 필드를 최상위 필드로 승격하는 방법을 확인해보자.  
`$필드명` 으로 각각의 필드에 접근하며, 중첩된 각 필드에 접근하기 위한 경로를 이어서 표현하면 된다.
...
출력된 결과는 다음과 같다.  
ipo, valuation은 단일 값으로 최상위 필드에 승격되었고, funders에는 `**.permalink`에 담긴 값들은 배열로 삽입되었다.  
...
집계 파이프라인을 이용해 배열 필드의 각 요소를 도큐먼트로 전개(unwind) 하는 것이 가능하다.  
예를 들어 배열의 크기가 3인 키를 기준으로 unwind를 수행하면, 각 요소를 값으로 가지는 세 갸의 도큐먼트가 생성된다.
...
이번에도 companies의 예시를 사용해서, 먼저 funding_rounds의 중첩된 필드들을 승격시킨 도큐먼트에서 예제를 시작해보자.  
funding_rounds.investments의 각 요소를 순회하면서 값을 승격시키므로, 중첩된 배열 형태로 필드가 완성된다.

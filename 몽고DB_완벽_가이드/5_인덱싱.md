...
몽고DB는 쿼리를 요청받으면 검색 및 정렬 필드를 구성하고 있는 쿼리 모양을 확인하고, 이를 기반으로 인덱스 후보들을 선정한다.  
각 인덱스 후보들로 쿼리 플랜이 구성되며, 병렬 쓰레드에서 개별적으로 실행된다.  
이렇게 모든 플랜이 한 번씩 실행 되는 것으로 히나의 레이스가 구성되고, 어떤 플랜이 가장 먼저 목표 상태에 도달했는지를 확인하게 된다.  
...
인덱스를 구성할 때에는 가능한 읽기/쓰기가 효율적으로 작동할 수 있도록 해야 한다.  
인덱스 설계 시에는 몇 가지 원칙들을 고려해야 하고, 실제 워크로드에서 인덱스를 테스트하고 조정하는 것이 좋다.

먼저 인덱스의 선택성을 고려해야 한다.  
인덱스를 구성할 때에는 특정 쿼리 패턴에서 레코드 스캔이 얼마나 최소화되는지를 확인한다.  
예를 들어 다음과 같은 형태의 도큐먼트에 대해서 인덱스를 구성한다고 하자.
...
이제 students 컬렉션에 대해서 다음과 같은 쿼리를 실행한다.  
student_id 범위 조건, class_id 일치 조건으로 결과를 제한하고, student_id로 정렬을 수행하는 쿼리이다.  
...
이는 인덱스가 선택적으로 작동하지 못했음을 의미하고, 총 실행 시간도 4.325초가 걸렸다.
...
쿼리 플랜은 위와 같이 단계 플랜으로 표현되며, 어떤 단계를 거쳐 플랜이 실행되었는지를 설명한다.  
...
예제에서는 복합 인덱스를 사용했으나 성능 향상은 제한적이었다.  
이는 student_id를 이용한 범위 조건이 낮은 선택성을 가지기 때문이다.  
student_id가 50000보다 큰 조건을 만족하는 도큐먼트는 85만개로 매우 많다.  
따라서 student_id가 인덱스의 첫번째 키로 포함되어 있더라도 85만개의 키를 모두 스캔해야 한다.  
...
다만 이런 식으로 인덱스를 직접 지정하면 상황에 따라 슬로우 쿼리를 유발할 수 있다.  
가능하다면 더 효과적으로 작동하는 인덱스를 추가로 등록하여 해당 인덱스가 사용되도록 유도하는 것이 적절하다.  
위 예제에서는 동등 필터에 사용되는 class_id를 첫번째 키로, 범위 쿼리에 사용되는 student_id를 두번째 키로 등록하면 된다.  
...
위와 같이 인덱스를 구성하면 다음과 같이 [class_id, student_id] 형태의 키 쌍으로 인덱스가 구성된다.  
각 class_id 별로 student_id가 정렬되어 있으므로, 예제 쿼리에서는 class_id가 54인 데이터에서 시작하여 데이터를 스캔하면 된다.
...
위 예제는 쿼리 필터와 정렬에 동일한 키가 사용된 특수한 케이스였다.  
복합 인덱스 설계 시에는 동등 필터, 다중값 필터, 정렬이 어떻게 동작하는지를 모두 고려해야 한다.  
이 점을 더 알아보기 위해 정렬에 final_grade를 사용하여 쿼리 필터와 정렬 키를 다르게 구성해보자.  
이제 실행 단계에 인메모리 정렬 단계가 포함되고, 쿼리 수행 시간이 0.136초로 증가한 것을 확인할 수 있다.
...
[동등 필터, 정렬 필터, 다중갑 필터] 형태로 인덱스를 구성하면 인메모리 정렬 없이 쿼리를 실행할 수 있다.
...
이제는 student_id 범위 조건에 맞는 데이터를 걸러내기 위해 class_id 동등 필터에 맞는 데이터를 모두 스캔해야 한다.
...
복합 키 인덱스를 구성할 때에는 해당 원칙들을 지켜서 구성하도록 하자.

- 동등 필터 키를 맨 앞에 표시
- 정렬 기준 키는 다중값 필터 키의 앞에 표시
- 다중값 필터 키는 마지막에 표시

#### 키 방향 선택하기

지금까지는 인덱스를 구성할 때 모두 오름차순으로 정렬했다.  
단일 키로 인덱스를 구성할 경우에는 인덱스의 방향과 무관하게 효과를 볼 수 있다.  
하지만 복합키로 구성할 때 다음처럼 서로 다른 방향을 함께 사용하여 정렬할 경우, 인덱스가 효율적으로 동작하지 않는다.

```js
db.users.find({}).sort({"username" : 1, age: -1})
```

따라서 복합 키 인덱스를 구성할 때에는 정렬의 방향까지 함께 고려해야 한다.  
다만 몽고DB는 인덱스의 역방향으로도 데이터를 스캔할 수 있으므로, 역방향인 인덱스를 중복해서 등록하지 않도록 주의하자.

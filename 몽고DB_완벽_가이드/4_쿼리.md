...
$where를 사용하면 임의의 자바스크립트 코드로 쿼리 조건을 넘길 수 있다.  
...
따라서 가능하면 다른 조건들은 인덱스를 통해 일반 조건으로 적용하고, 정말 필요한 부분만 $where를 이용하여 쿼리를 구성해야 한다.  
또한 집계 표현식을 사용하기 위해서 $where를 사용하는 것이라면, $expr을 사용하여 대체하는 것도 가능하다.
...
findOne에 $lt 또는 $gt로 랜덤 값 조건을 주는 식으로 데이터를 추출할 수 있다.  
